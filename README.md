# 🌦️ Weather Full Stack — Coletor, Fila, API e Dashboard

Este projeto é uma solução completa de monitoramento climático composta por:

- **Python Collector** → coleta dados meteorológicos da API Open-Meteo
- **Go Producer + RabbitMQ** → recebe dados e envia para a fila
- **NestJS API + MongoDB** → consome da fila, persiste e expõe endpoints REST
- **React + Vite + Tailwind + Shadcn** → dashboard moderno com estatísticas em tempo real
- **Docker Compose** → orquestra todos os serviços

## 📦 Arquitetura Geral

Python Collector → Go Producer → RabbitMQ → NestJS API → MongoDB → React Frontend
...
## 🐍 Como rodar serviço Python

```sh
python app.py
```

## 🐹 Como rodar worker Go

```sh
go run main.go
```

⚠️ Inicialização do Replica Set do MongoDB

Quando você utiliza volumes (-v) no serviço MongoDB dentro do Docker Compose, o Replica Set não inicia automaticamente.
Sem ele, o NestJS não conseguirá se conectar ao banco no modo replicaSet.

Após subir os containers, execute o comando abaixo dentro do container do MongoDB:
