...
const TemperatureChart = ({ data, viewMode = 'hour', setFilters, date, isLoading }: TemperatureChartProps) => {
    ...
    <Card className="p-6 bg-card border-border shadow-lg hover:shadow-xl transition-shadow duration-300">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold text-card-foreground">Temperatura</h2>
            <p className="text-xs text-muted-foreground">°C ao longo do tempo</p>
          </div>
        </div>
        <div className="flex flex-col md:flex-row items-center gap-4">
          ...
          {hasData && <Tabs value={viewMode} onValueChange={(v) => handleViewMode(v as 'hour' | 'day')}>
           <TabsList>
            <TabsTrigger value="hour" asChild>
              <div role='button' className='cursor-pointer'>Por Hora</div>
            </TabsTrigger>
            <TabsTrigger value="day" asChild>
              <div role='button' className='cursor-pointer'>Por Dia</div>
            </TabsTrigger>
          </TabsList>
        </Tabs>}
        </div>
      ...
      {!isLoading && <LineChartComponent data={data} xAxisKey={viewMode === 'hour' ? "hour" : "date"} dataKey="temperature" yAxisLabel="°C" />}
    </Card>
  );
};
